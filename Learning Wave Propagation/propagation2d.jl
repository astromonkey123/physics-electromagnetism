...
function E(point_pos, charge::point_charge, t)
    pos = charge.pos
    q = charge.q

    radius = hypot(point_pos[1] - pos(t)[1], point_pos[2] - pos(t)[2])

    t_delayed = t - (radius / c)
    radius_delayed = hypot(point_pos[1] - pos(t_delayed)[1], point_pos[2] - pos(t_delayed)[2])

    # u = -q * (1 / radius) * (1 / radius_delayed) * (point_pos[1] - pos(t_delayed)[1])
    # v = -q * (1 / radius) * (1 / radius_delayed) * (point_pos[2] - pos(t_delayed)[2])

    u = -q * (1 / radius_delayed) * (point_pos[1] - pos(t_delayed)[1])
    v = -q * (1 / radius_delayed) * (point_pos[2] - pos(t_delayed)[2])
